...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # csv_path = r"G:\New_workstations\Course\zongsan\yolov5_for_course3\save_csv" # 添加路径存储csv文件
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop:
                        ...
                        last_save_path = save_dir / "crops"
                        # Save cropped image
                        # save_crop_dir = save_dir / "crops" / names[c]
                        # print("save_crop_dir:" + save_crop_dir)
...
def parse_opt(file_path):
    ...
    # 将结果保存为*.txt文件
    parser.add_argument("--save-txt", action="True", help="save results to *.txt")
    # 将结果保存为CSV格式
    ...
    # 不保存图像或视频
    parser.add_argument("--no-save", action="store_true", help="do not save images_for_test/videos")
    ...
    # 保存结果的项目路径
    parser.add_argument("--project", default=ROOT / "test", help="save results to project/name")
    # 保存结果的文件夹名称
    parser.add_argument("--name", default="exp_test", help="save results to project/name")
    # 如果项目/名称存在，则不增加后缀
...
# file_path = r"G:\New_workstations\Course\zongsan\images_t\images_for_test"


def main(file_path):
    opt = argparse.Namespace(
        weights=[str(ROOT / "best_300.pt")],  # 模型权重文件路径
        source=str(ROOT / file_path),  # 数据源路径
        data=str(ROOT / "Maskdata/mydata.yaml"),  # 数据集配置文件路径
        imgsz=[640,640],  # 输入图片大小
        conf_thres=0.25,  # 置信度阈值
        iou_thres=0.45,  # NMS IoU 阈值
        max_det=1000,  # 每张图片的最大检测数量
        device="0",  # 使用的设备
        view_img=False,  # 是否显示结果图片
        save_txt=True,  # 是否保存结果到 *.txt 文件
        save_csv=False,  # 是否保存结果到 CSV 文件
        save_conf=False,  # 是否在保存的标签中包含置信度
        save_crop=True,  # 是否保存裁剪后的预测框
        nosave=False,  # 是否不保存图片或视频
        classes=None,  # 类别过滤
        agnostic_nms=False,  # 类别无关的 NMS
        augment=False,  # 增强推理
        visualize=False,  # 可视化特征
        update=False,  # 更新所有模型
        project=str(ROOT / r"G:\New_workstations\Course\zongsan\Detected_images"),  # 保存结果的项目路径
        name="exp_test",  # 保存结果的文件夹名称
        exist_ok=False,  # 如果项目/名称存在则不增加后缀
        line_thickness=1,  # 边界框的线条粗细（像素）
...
# opt = parse_opt(r"G:\New_workstations\Course\zongsan\images_t\images_for_test")
# 测试文件(opt)
# 创建一个 argparse.Namespace 对象来模拟命令行参数

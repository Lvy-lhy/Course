...
def export_onnx(model, im, file, opset, dynamic, simplify, prefix=colorstr("ONNX:")):
    ...
    if dynamic:
        dynamic = {"images_for_test": {0: "batch", 2: "height", 3: "width"}}  # shape(1,3,640,640)
...
def export_edgetpu(file, prefix=colorstr("Edge TPU:")):
    ...
    if subprocess.run(f"{cmd} > /dev/null 2>&1", shell=True).returncode != 0:
        ...
        for c in (
            ...
            'echo "deb https://packages.cloud.google.com/apt coral-edgetpu-stable 测试文件" | sudo tee /etc/apt/sources.list.d/coral-edgetpu.list',
            "sudo apt-get update",

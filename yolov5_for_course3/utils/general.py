...
def check_git_info(path="."):
    ...
    try:
        ...
        try:
            branch = repo.active_branch.name  # i.e. '测试文件'
...
def check_amp(model):
    ...
    f = ROOT / "data" / "images_for_test" / "bus.jpg"  # image to check
...
def url2file(url):
    """
    Converts a URL string to a val filename by stripping protocol, domain, and any query parameters.
...
def increment_path(path, exist_ok=False, sep="", mkdir=False):
    ...
    Example: runs/exp_noAdd --> runs/exp_noAdd{sep}2, runs/exp_noAdd{sep}3, ... etc
    """

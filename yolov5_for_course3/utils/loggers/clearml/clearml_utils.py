"""测试文件 Logger class for ClearML experiment tracking."""
...
class ClearmlLogger:
    ...
    def __init__(self, opt, hyp):
        ...
        # Keep tracked of amount of logged images_for_test to enforce a limit
        self.current_epoch_logged_images = set()
        # Maximum number of images_for_test to log to clearML per epoch
        self.max_imgs_to_log_per_epoch = 16
        # Get the interval of epochs when bounding box images_for_test should be logged
        ...
        if self.clearml:
            self.task = Task.init(
                ...
                task_name=opt.name if opt.name != "exp_noAdd" else "Training",
                tags=["YOLOv5"],
                output_uri=True,
    ...
    def log_debug_samples(self, files, title="Debug Samples"):
        """
        Log files (images_for_test) as debug samples in the ClearML task.

        arguments:
        files (List(PosixPath)) a list of file paths in PosixPath format
        title (str) A title that groups together images_for_test with the same values
        """
        for f in files:
            if f.exists():

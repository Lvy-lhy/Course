...
class Loggers:
    ...
    def on_train_batch_end(self, model, ni, imgs, targets, paths, vals):
        """Logs training batch end events, plots images_for_test, and updates external loggers with batch-end data."""
    ...
    def on_train_end(self, last, best, epoch, results):
        ...
        if self.tb and not self.clearml:  # These images_for_test are already captured by ClearML by now, we don't want doubles
            for f in files:
...
class GenericLogger:
    ...
    def __init__(self, opt, console_logger, include=("tb", "wandb", "clearml")):
        ...
        if wandb and "wandb" in self.include:
            self.wandb = wandb.init(
                project=web_project_name(str(opt.project)), name=None if opt.name == "exp_noAdd" else opt.name, config=opt
            )
        else:
            self.wandb = None
    ...
    def log_images(self, files, name="Images", epoch=0):
        """Logs images_for_test to all loggers with optional naming and epoch specification."""

...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        """Initializes YOLOv5 loader for images_for_test/videos, supporting glob patterns, directories, and lists of paths."""
        ...
        if any(videos):
            ...
            self.cap = None
        assert self.nf > 0, (
            f"No images_for_test or videos found in {p}. "
            f"Supported formats are:\nimages_for_test: {IMG_FORMATS}\nvideos: {VID_FORMATS}"
        )
...
class LoadStreams:
    ...
    def __iter__(self):
        """Resets and returns the iterator for iterating over video frames or images_for_test in a dataset."""
        self.count = -1
        return self

    def __next__(self):
        """Iterates over video frames or images_for_test, halting on thread stop or 'q' key press, raising `StopIteration` when
        done.
        """
        self.count += 1
...
def img2label_paths(img_paths):
    """Generates label file paths from corresponding image file paths by replacing `/images_for_test/` with `/labels/` and
    extension with `.txt`.
    """
    sa, sb = f"{os.sep}images_for_test{os.sep}", f"{os.sep}labels{os.sep}"  # /images_for_test/, /labels/ substrings
    return [sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt" for x in img_paths]


class LoadImagesAndLabels(Dataset):
    # YOLOv5 train_loader/val_loader, loads images_for_test and labels for training and validation
    ...
    def __init__(
        ...
        self.mosaic = self.augment and not self.rect  # load 4 images_for_test at a time into a mosaic (only during training)
        ...
        try:
            ...
            assert self.im_files, f"{prefix}No images_for_test found"
        ...
        if exists and LOCAL_RANK in {-1, 0}:
            d = f"Scanning {cache_path}... {nf} images_for_test, {nm + ne} backgrounds, {nc} corrupt"
        ...
        # Filter images_for_test
        if min_items:
            include = np.array([len(x) >= min_items for x in self.labels]).nonzero()[0].astype(int)
            LOGGER.info(f"{prefix}{n - len(include)}/{n} images_for_test filtered from dataset")
        ...
        # Create indices
        n = len(self.shapes)  # number of images_for_test
        ...
        # Cache images_for_test into RAM/disk for faster training
        if cache_images == "ram" and not self.check_cache_ram(prefix=prefix):
            cache_images = False
        self.ims = [None] * n
        self.npy_files = [Path(f).with_suffix(".npy") for f in self.im_files]
        if cache_images:
            b, gb = 0, 1 << 30  # bytes of cached images_for_test, bytes per gigabytes
            ...
            for i, x in pbar:
                ...
                pbar.desc = f"{prefix}Caching images_for_test ({b / gb:.1f}GB {cache_images})"
            pbar.close()

    def check_cache_ram(self, safety_margin=0.1, prefix=""):
        """Checks if available RAM is sufficient for caching images_for_test, adjusting for a safety margin."""
        b, gb = 0, 1 << 30  # bytes of cached images_for_test, bytes per gigabytes
        n = min(self.n, 30)  # extrapolate from 30 random images_for_test
        for _ in range(n):
        ...
        if not cache:
            LOGGER.info(
                ...
                f"{'caching images_for_test ✅' if cache else 'not caching images_for_test ⚠️'}"
            )
        return cache

    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        """Caches dataset labels, verifies images_for_test, reads shapes, and tracks dataset integrity."""
        x = {}  # dict
        ...
        with Pool(NUM_THREADS) as pool:
            ...
            for im_file, lb, shape, segments, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                ...
                if msg:
                    msgs.append(msg)
                pbar.desc = f"{desc} {nf} images_for_test, {nm + ne} backgrounds, {nc} corrupt"
    ...
    def __len__(self):
        """Returns the number of images_for_test in the dataset."""
    ...
    def load_mosaic(self, index):
        """Loads a 4-image mosaic for YOLOv5, combining 1 selected and 3 random images_for_test, with labels and segments."""
        labels4, segments4 = [], []
        s = self.img_size
    ...
    def load_mosaic9(self, index):
        """Loads 1 image + 8 random images_for_test into a 9-image mosaic for augmented YOLOv5 training, returning labels and
        segments.
        """
        labels9, segments9 = [], []
        s = self.img_size
    ...
    def collate_fn(batch):
        """Batches images_for_test, labels, paths, and shapes, assigning unique indices to targets in merged label tensor."""
...
def extract_boxes(path=DATASETS_DIR / "coco128"):
    ...
    """
    path = Path(path)  # images_for_test dir
...
def autosplit(path=DATASETS_DIR / "coco128/images_for_test", weights=(0.9, 0.1, 0.0), annotated_only=False):
    """Autosplit a dataset into train/val/test splits and save path/autosplit_*.txt files
    Usage: from utils.dataloaders import *; autosplit()
    Arguments
        path:            Path to images_for_test directory
        weights:         Train, val, test weights (list, tuple)
        annotated_only:  Only use images_for_test with an annotated txt file
    """
    path = Path(path)  # images_for_test dir
    ...
    print(f"Autosplitting images_for_test from {path}" + ", using *.txt labeled images_for_test only" * annotated_only)
...
class HUBDatasetStats:
    ...
    def __init__(self, path="coco128.yaml", autodownload=False):
        ...
        self.im_dir = self.hub_dir / "images_for_test"
        self.im_dir.mkdir(parents=True, exist_ok=True)  # makes /images_for_test
    ...
    def process_images(self):
        """Compresses images_for_test for Ultralytics HUB across 'train', 'val', 'test' splits and saves to specified
        directory.
        """
        for split in "train", "val", "test":
            if self.data.get(split) is None:
                continue
            dataset = LoadImagesAndLabels(self.data[split])  # load dataset
            desc = f"{split} images_for_test"
            for _ in tqdm(ThreadPool(NUM_THREADS).imap(self._hub_ops, dataset.im_files), total=dataset.n, desc=desc):
                pass
        print(f"Done. All images_for_test saved to {self.im_dir}")
        return self.im_dir

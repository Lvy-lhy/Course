...
def normalize(x, mean=IMAGENET_MEAN, std=IMAGENET_STD, inplace=False):
    """
    Applies ImageNet normalization to RGB images_for_test in BCHW format, modifying them in-place if specified.
...
def denormalize(x, mean=IMAGENET_MEAN, std=IMAGENET_STD):
    """Reverses ImageNet normalization for BCHW format RGB images_for_test by applying `x = x * std + mean`."""
    for i in range(3):
...
def mixup(im, labels, im2, labels2):
    """
    Applies MixUp augmentation by blending images_for_test and labels.

...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if pt:  # PyTorch
            ...
            batch_size = bindings["images_for_test"].shape[0]  # if dynamic, this is instead max batch size
        elif coreml:  # CoreML
    ...
    def forward(self, im, augment=False, visualize=False):
        """Performs YOLOv5 inference on input images_for_test with options for augmentation and visualization."""
        ...
        if self.pt:  # PyTorch
            ...
            if self.dynamic and im.shape != self.bindings["images_for_test"].shape:
                i = self.model.get_binding_index("images_for_test")
                self.context.set_binding_shape(i, im.shape)  # reshape if dynamic
                self.bindings["images_for_test"] = self.bindings["images_for_test"]._replace(shape=im.shape)
                for name in self.output_names:
                    i = self.model.get_binding_index(name)
                    self.bindings[name].data.resize_(tuple(self.context.get_binding_shape(i)))
            s = self.bindings["images_for_test"].shape
            assert im.shape == s, f"input size {im.shape} {'>' if self.dynamic else 'not equal to'} max model size {s}"
            self.binding_addrs["images_for_test"] = int(im.data_ptr())
...
class AutoShape(nn.Module):
    ...
    def forward(self, ims, size=640, augment=False, profile=False):
        ...
        """
        # For size(height=640, width=1280), RGB images_for_test example inputs are:
        #   file:        ims = 'data/images_for_test/zidane.jpg'  # str or PosixPath
        #   URI:             = 'https://ultralytics.com/images/zidane.jpg'
        #   OpenCV:          = cv2.imread('image.jpg')[:,:,::-1]  # HWC BGR to RGB x(640,1280,3)
        #   PIL:             = Image.open('image.jpg') or ImageGrab.grab()  # HWC x(640,1280,3)
        #   numpy:           = np.zeros((640,1280,3))  # HWC
        #   torch:           = torch.zeros(16,3,320,640)  # BCHW (scaled to size=640, 0-1 values)
        #   multiple:        = [Image.open('image1.jpg'), Image.open('image2.jpg'), ...]  # list of images_for_test
        ...
        with dt[0]:
            ...
            # Pre-process
            n, ims = (len(ims), list(ims)) if isinstance(ims, (list, tuple)) else (1, [ims])  # number, list of images_for_test
...
class Detections:
    ...
    def __init__(self, ims, pred, files, times=(0, 0, 0), names=None, shape=None):
        ...
        self.ims = ims  # list of images_for_test as numpy arrays
        ...
        self.n = len(self.pred)  # number of images_for_test (batch size)
    ...
    @TryExcept("Showing images_for_test is not supported in this environment")
    ...
    def save(self, labels=True, save_dir="runs/detect/exp_noAdd", exist_ok=False):
        """
        Saves detection results with optional labels to a specified directory.

        Usage: save(labels=True, save_dir='runs/detect/exp_noAdd', exist_ok=False)
        """
        save_dir = increment_path(save_dir, exist_ok, mkdir=True)  # increment save_dir
        self._run(save=True, labels=labels, save_dir=save_dir)  # save results

    def crop(self, save=True, save_dir="runs/detect/exp_noAdd", exist_ok=False):
        """
    ...
    def render(self, labels=True):
        """Renders detection results with optional labels on images_for_test; args: labels (bool) indicating label inclusion."""

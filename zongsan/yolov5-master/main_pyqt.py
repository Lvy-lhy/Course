...
from PyQt5.QtWidgets import QLabel, QVBoxLayout, QWidget, QPushButton, QFileDialog, QListWidget, QListWidgetItem, \
...
class App(QWidget):
    ...
    def initUI(self):
        ...
        self.btn_crop_images = QPushButton('Crop Images', self)
        self.btn_crop_images.clicked.connect(self.crop_images)
        layout.addWidget(self.btn_crop_images)

        self.btn_predict = QPushButton('Predict Images', self)
    ...
    def select_folder(self):
        self.source_folder = QFileDialog.getExistingDirectory(self, "Select Directory")
    ...
    def predict_images(self):
        ...
        for category in categories:
            ...
            if os.path.exists(category_dir):
                self.text_edit.append(f"Processing category: {category}")
                for image_name in os.listdir(category_dir):
                    image_path = os.path.join(category_dir, image_name)
                    if image_path.endswith(('.png', '.jpg', '.jpeg')):

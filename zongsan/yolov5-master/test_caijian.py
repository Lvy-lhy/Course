...
# Add a function to save cropped images
def save_cropped(img, bbox, save_path, name):
    x1, y1, x2, y2 = int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])
    cropped_img = img[y1:y2, x1:x2]
    cv2.imwrite(os.path.join(save_path, name), cropped_img)
...
if __name__ == '__main__':
    ...
    img_size = check_img_size(opt.img_size, s=stride)  # check img_size
    if half:
        model.half()  # to FP16
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            if opt.source.isnumeric():  # batch_size >= 1
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if opt.save_img or opt.save_crop:  # Add bbox to image
                        label = f'{names[int(cls)]} {conf:.2f}'
                        plot_one_box(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=opt.line_thickness)

                        # Save cropped image
                        if opt.save_crop:
                            save_cropped(im0, xyxy, save_dir / 'crops', f'{Path(p).stem}_{int(cls)}.jpg')

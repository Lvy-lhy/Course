...
from PyQt5 import QtWidgets, QtGui, QtCore  # Corrected import for QtCore
...
class App(QtWidgets.QWidget):  # 继承自 QtWidgets.QWidget
    ...
    def initUI(self):
        ...
        self.btn_crop_images = QtWidgets.QPushButton('Crop Images', self)
        self.btn_crop_images.clicked.connect(self.crop_images)
        layout.addWidget(self.btn_crop_images)

        self.btn_predict = QtWidgets.QPushButton('Predict Images', self)
        self.btn_predict.clicked.connect(self.predict_images)
        layout.addWidget(self.btn_predict)

        # 创建图片显示标签和文本框
        self.image_label = QtWidgets.QLabel(self)
        self.image_label.setAlignment(QtCore.Qt.AlignCenter)  # 使用 QtCore.Qt.AlignCenter
        ...
        # 创建导航按钮
        nav_layout = QtWidgets.QHBoxLayout()  # 使用 QtWidgets.QHBoxLayout
        self.btn_prev = QtWidgets.QPushButton('Previous', self)
    ...
    def select_folder(self):
        self.source_folder = QtWidgets.QFileDialog.getExistingDirectory(self, "Select Directory")
        if self.source_folder:
            print(f"Selected folder: {self.source_folder}")
    ...
    def crop_images(self):
        ...
        if not detect_dirs:
            print("No detect directories found.")
            return
        ...
        if not os.path.exists(crops_dir):
            print(f"Crops directory does not exist: {crops_dir}")
            return
    ...
    def predict_images(self):
        if not self.target_dir:
            print("Please crop images first")
            return
        ...
        if not self.image_list:
            print("No images found in target directory.")
            return
    ...
    def predict_image(self, image_path):
        ...
        label_explanation = label_map[predicted_label]  # 返回标签对应的中文解释
    ...
    def show_image_and_predictions(self, index):
        if 0 <= index < len(self.image_list):
            ...
            self.text_edit.append(f"Image: {prediction['image_name']}")
            self.text_edit.append(f"Category: {prediction['category']}")
            self.text_edit.append(f"Predicted Label: {prediction['predicted_label']}")
            self.text_edit.append(f"Label Explanation: {prediction['label_explanation']}")
        else:
            self.image_label.clear()
            self.text_edit.clear()
            self.text_edit.append("No image to display.")
